...
const { reducer: bondsReducer } = createSlice({
	...
	extraReducers: {
		...
		[bondsAsyncActions.destroy.fulfilled.type]: (state, action: CPA<{ bondId: string }>) => {
			const { bondId } = action.payload;
			const list = state.list.filter((bond) => bond._id !== bondId);
			state.list = list;
			state.updatedAt = Date.now();
			action.dispatch(
				requestActions.fulfilled({
					name: bondsAsyncActions.destroy.typePrefix,
					message: '',
					payload: {},
				})
			);
		},
		[bondsAsyncActions.destroy.rejected.type]: (state, { payload, dispatch }: CPA<ErrorResponse>) => {
			if (payload.error.status === 401) {
				state = initialState;
			}
